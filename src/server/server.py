    ...
    try:
        ...
        response = {
            config.PAYLOAD_KEY_CLIENTS: (
                clients_data if clients_data is not None else {}
            ),
            config.STATUS_KEY_REDIS_STATUS: redis_status_str
            or config.STATUS_VALUE_REDIS_UNKNOWN,
        }
...
async def health_check():
    ...
    return {
        ...
        config.STATUS_KEY_REDIS_STATUS: status_store.get(
            config.STATUS_KEY_REDIS_STATUS, config.STATUS_VALUE_REDIS_UNKNOWN
        ),
    ...
    try:
        ...
        try:
            ...
            await websocket.close(
                code=config.WEBSOCKET_CLOSE_CODE_INVALID_PAYLOAD
            )  # Ensure this constant exists
            return

        client_id = message.get(config.PAYLOAD_KEY_CLIENT_ID)
        status_attributes = message.get(config.PAYLOAD_KEY_STATUS, {})
        client_role = status_attributes.get(
            config.STATUS_KEY_CLIENT_ROLE
        )  # Expect role in status
        ...
        if stored_status_after_conn is None:
            ...
            stored_status_after_conn[config.STATUS_KEY_REDIS_STATUS] = (
                config.STATUS_VALUE_REDIS_UNAVAILABLE
            )
            # Ensure client_role is present from the registration step
            if (
                config.STATUS_KEY_CLIENT_ROLE not in stored_status_after_conn
                and client_role
            ):
            ...
            if (
                client_role
                and stored_status_after_conn.get(config.STATUS_KEY_CLIENT_ROLE)
                != client_role
            ):
                stored_status_after_conn[config.STATUS_KEY_CLIENT_ROLE] = client_role
        ...
        if client_role == config.CLIENT_ROLE_FRONTEND:
            ...
            all_clients_data, redis_status_val, error = (
                await get_all_client_statuses()
            )  # Renamed redis_status to avoid conflict
        ...
        while True:
            ...
            if message_type == config.MSG_TYPE_CONTROL:
                ...
                control_message_id = message.get(
                    config.PAYLOAD_KEY_MESSAGE_ID
                ...
                if action == config.CONTROL_ACTION_PAUSE:
                    response_payload = await pause_client(
                        target_client_id, client_id
                    )  # Pass originating client_id
                elif action == config.CONTROL_ACTION_RESUME:
                    response_payload = await resume_client(
                        target_client_id, client_id
                    )  # Pass originating client_id
                elif action == config.CONTROL_ACTION_DISCONNECT:
                    response_payload = await disconnect_client(
                        target_client_id, client_id
                    )  # Pass originating client_id
                else:
                    response_payload = {
            ...
            # For now, assume any message not of type 'control' from a non-frontend client is a status update.
                ...
                message_client_id_from_payload = message.get(
                    config.PAYLOAD_KEY_CLIENT_ID
                )
                if (
                ...
                if not isinstance(status_attributes, dict):
                    ...
                    status_attributes = {
                        config.STATUS_KEY_RAW_PAYLOAD: status_attributes
                    }
        ...
        if websocket.client_state == WebSocketState.CONNECTED:
            try:
                await websocket.send_text(
                    json.dumps({config.MSG_TYPE_ERROR: error_msg})
                )
        ...
        if client_id:  # client_id is the one captured from registration or message loop
            await handle_disconnect(
                client_id, websocket, config.REASON_DISCONNECTED_BY_CLIENT
            )
        else:
...
async def get_client_info(client_id: str) -> dict | None:
    ...
    if (
        status_store.get(config.STATUS_KEY_REDIS_STATUS)
    ...
    try:
        ...
        if not status_data:
            logger.debug(
                f"No status data found in Redis for client {client_id} with key {redis_key}"
            )
            return None
        ...
        logger.debug(
            f"Decoded status for {client_id}: {list(status.keys())}"
        )  # Log keys for easier debug
        return status
    except Exception as e:  # More specific exception handling could be added if needed
        logger.error(
            f"Error getting client info for {client_id} from Redis: {e}", exc_info=True
        )
        return None
    ...
    try:
        ...
        if broadcast:
            ...
            await broadcast_client_state_change(
                client_id, broadcast_attrs, originating_client_id=originating_client_id
            )
...
async def disconnect_client(
    target_client_id: str, originating_client_id: str | None = None
) -> dict:
    logger.info(
        f"Processing disconnect request for client {target_client_id}, initiated by {originating_client_id or 'server'}"
    )
    ...
    if (
        ...
        and client_info_pre_disconnect.get(config.STATUS_KEY_CONNECTED)
        == config.STATUS_VALUE_DISCONNECTED
    ):
        client_already_disconnected = True
        logger.info(
            f"Client {target_client_id} was already marked as disconnected in Redis."
        )
    ...
    current_redis_status = status_store.get(
        config.STATUS_KEY_REDIS_STATUS, config.STATUS_VALUE_REDIS_UNKNOWN
    )
    ...
    if not (closed_worker or closed_frontend) and not client_already_disconnected:
        ...
        status_key = (
            "success"  # This remains a string literal as per original, not a constant
        )
        logger.info(message)
...
async def pause_client(
    ...
    logger.info(
        f"Processing pause request for client {target_client_id}, initiated by {originating_client_id or 'server'}"
    )
    ...
    target_client_exists_and_connected = (
        ...
        and client_info.get(config.STATUS_KEY_CONNECTED)
    ...
    if not websocket:
        ...
        try:
            await websocket.send_text(
                json.dumps({config.PAYLOAD_KEY_COMMAND: config.CONTROL_ACTION_PAUSE})
            )
            logger.info(
                f"Pause command sent to client {target_client_id} via WebSocket"
            )
            sent_command = True
        except (WebSocketDisconnect, RuntimeError) as e:  # pragma: no cover
            logger.warning(
            ...
            logger.error(
                f"Error sending pause command to client {target_client_id}: {e}",
                exc_info=True,
            )
            # worker_connections.pop(target_client_id, None) # Let main handler deal with it
...
async def resume_client(
    ...
    logger.info(
        f"Processing resume request for client {target_client_id}, initiated by {originating_client_id or 'server'}"
    )
    websocket = worker_connections.get(target_client_id)
    ...
    if not websocket:
        ...
        try:
            await websocket.send_text(
                json.dumps({config.PAYLOAD_KEY_COMMAND: config.CONTROL_ACTION_RESUME})
            )
            logger.info(
                f"Resume command sent to client {target_client_id} via WebSocket"
            )
            sent_command = True
        except (WebSocketDisconnect, RuntimeError) as e:  # pragma: no cover
            logger.warning(
            ...
            logger.error(
                f"Error sending resume command to client {target_client_id}: {e}",
                exc_info=True,
            )
            # worker_connections.pop(target_client_id, None) # Let main handler deal with it
            return {
...
async def handle_disconnect(client_id: str, websocket: WebSocket, reason: str):
    ...
    if not processed_disconnect:
        ...
        if (
            ...
            and client_info_if_exists.get(config.STATUS_KEY_CONNECTED)
            == config.STATUS_VALUE_CONNECTED
        ):
            logger.warning(
            ...
            client_role_for_log = client_info_if_exists.get(
                config.STATUS_KEY_CLIENT_ROLE, config.CLIENT_ROLE_WORKER
            )  # Default to worker for broadcast decision
