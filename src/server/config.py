...
WEBSOCKET_CLOSE_CODE_CLIENT_DISCONNECT = (
    1000  # Standard code for normal closure by client
)
WEBSOCKET_CLOSE_CODE_NORMAL_CLOSURE = 1000  # Standard code for normal closure
...
WEBSOCKET_CLOSE_CODE_INVALID_PAYLOAD = (
    1007  # Consistent with CloseCode.INVALID_FRAME_PAYLOAD_DATA
)
...
REDIS_PORT = 6379
REDIS_DB = 0
REDIS_CLIENT_INFO_KEY_PREFIX = (
    "client_info:"  # Prefix for keys storing client information
)
...
REDIS_RECONNECT_ATTEMPT_INTERVAL_SECONDS = (
    10  # Interval between Redis reconnect attempts
)
...
STATUS_VALUE_REDIS_CONNECTED = "connected"
...
PAYLOAD_KEY_ERROR_REDIS = (
    "error_redis"  # Specific error key for redis issues in status response
)
PAYLOAD_KEY_STATUS_UPDATED = (
    "status_updated"  # For worker status update acknowledgements
)
...
ERROR_MSG_CONTROL_PERMISSION_DENIED = (
    "Permission denied: Only frontend clients can issue control commands."
)
ERROR_MSG_CONTROL_INVALID_PAYLOAD = (
    "'action' and 'target_client_id' are required for control commands."
)
ERROR_MSG_INVALID_JSON = "Invalid JSON format"
INFO_MSG_UNHANDLED_FRONTEND_MSG = (
    "Message type not recognized for frontend clients, aside from 'control'."
)
