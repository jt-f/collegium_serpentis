...
@pytest.mark.asyncio
async def test_hml_routes_to_chat_message(
    manager: ConnectionManager, mock_ws: AsyncMock, monkeypatch
):
    ...
    try:
        await manager._handle_message_loop(
            mock_ws, "frontend1", server_config.CLIENT_ROLE_FRONTEND
        )
...
@pytest.mark.asyncio
async def test_hml_chat_message_not_frontend_goes_to_unknown(
    manager: ConnectionManager, mock_ws: AsyncMock, monkeypatch
):
    """Test that chat messages from non-frontend clients are handled as unknown."""
    mock_handle_unknown = AsyncMock()
    monkeypatch.setattr(
        manager, "_handle_unknown_frontend_message", mock_handle_unknown
    )
...
async def test_handle_chat_message_happy_path(
    ...
    # Verify logging
    mock_logger.info.assert_any_call(
        "Received chat message from frontend client frontend1"
    )
    mock_logger.info.assert_any_call(
        "Chat message from frontend1: Hello, this is a test message"
    )
...
async def test_handle_chat_message_empty_message(
    ...
    mock_logger.warning.assert_called_with(
        "Empty chat message received from client frontend1"
    )
...
async def test_handle_chat_message_long_message_truncated_log(
    ...
    message = {"type": "chat", "client_id": "frontend1", "message": long_message}
...
async def test_handle_chat_message_no_timestamp_auto_generated(
    manager: ConnectionManager, mock_ws: AsyncMock, mock_redis_manager: AsyncMock
):
...
async def test_handle_chat_message_redis_status_included(
    ...
    message = {"type": "chat", "client_id": "frontend1", "message": "Test message"}
